...
function applyAccepted(container: HTMLElement, findings: readonly AuditFinding[], decisions: AuditState['decisions']): void {
  ...
  for (const finding of findings) {
    ...
    replaceQuote(container, finding, parseIssue(finding.issue).severity)
  }
}
...
/** Opening markup of the plain-text preview block; the escaped text and its closing tag follow. */
const PRE_OPEN = '<pre style="white-space: pre-wrap; font-family: inherit; margin: 0;">'

/**
 * Wrap decoded plain text as escaped preformatted HTML, the panel's rendering for a non-HTML source.
 * @param text - the decoded file text.
 * @returns HTML carrying the text with newlines preserved.
 */
...
function BasisRow({ citations, sessionId, readFileBytes, t }: {
  ...
  // Group articles by regulation title so the same regulation is not repeated.
  const grouped = useMemo(() => {
    if (!articles) return []
    const map = new Map<string, { title: string; texts: string[] }>()
    for (const article of articles) {
      const existing = map.get(article.path)
      if (existing) {
        if (!existing.texts.includes(article.text)) existing.texts.push(article.text)
      } else {
        map.set(article.path, { title: article.title, texts: [article.text] })
      }
    }
    return [...map.values()]
  }, [articles])
  ...
  return (
    <div style={{ marginBottom: '8px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', paddingLeft: '10px', fontSize: '11px', lineHeight: 1.5 }}>
        <span style={{ color: 'rgb(23, 92, 211)', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {t('finding.basis')}：{[...new Set(sources.map(citationLabel))].join('、')}
        </span>
      ...
      {open && (
        <div style={{
          ...
          fontSize: '11px', lineHeight: 1.6, color: 'rgb(70, 80, 100)', maxHeight: '150px',
        }}>
          {loading && <span>{t('finding.basisLoading')}</span>}
          {!loading && articles !== null && articles.length === 0 && <span>{t('finding.basisMissing')}</span>}
          {grouped.map((group, at) => (
            <div key={at} style={{ marginTop: at === 0 ? 0 : '8px' }}>
              <div style={{ fontWeight: 600, color: 'rgb(30, 35, 45)' }}>{group.title}</div>
              {group.texts.map((text, i) => (
                <div key={i} style={{ whiteSpace: 'pre-wrap', marginTop: i > 0 ? '6px' : 0, paddingTop: i > 0 ? '6px' : 0, borderTop: i > 0 ? '1px solid rgb(222, 230, 242)' : 'none' }}>{text}</div>
              ))}
            </div>
          ))}
        </div>
...
/** Preview failure to render: the dictionary key naming it and, for a load failure, the transport message. */
type DocError = { key: 'preview.noHtml' | 'preview.binary' | 'preview.loadFailed'; message?: string }
...
export function ReviewPanel({ sessionId, useProjection, readFileBytes, t, onDecide, onApply }: ReviewPanelProps) {
  ...
  // Load the anchor's HTML preview and fall back to the anchor itself.
  useEffect(() => {
    if (!state || !anchorPath || activeView !== 'text') return
    setDocHtml('')
    setDocError(null)

    const failLoad = (error: unknown): void => {
      setDocError({ key: 'preview.loadFailed', message: error instanceof Error ? error.message : String(error) })
    }
    // The anchor's own rendering: as HTML when the anchor is one, as escaped text otherwise.
    const showAnchor = (): void => {
      if (BINARY_EXTENSIONS.test(anchorPath)) {
        setDocError({ key: 'preview.binary' })
        return
      }
      readFileBytes(sessionId, anchorPath)
        .then((bytes) => {
          const text = new TextDecoder().decode(bytes)
          setDocHtml(/\.html?$/i.test(anchorPath) ? DOCUMENT_STYLES + text : preformatted(text))
        })
        .catch(failLoad)
    }

    // Try loading the HTML version first
    if (htmlPath === null || htmlPath === anchorPath) {
      // No HTML derivation possible, render anchor.path directly
      showAnchor()
      return
    }
    readFileBytes(sessionId, htmlPath)
      .then(bytes => setDocHtml(DOCUMENT_STYLES + new TextDecoder().decode(bytes)))
      .catch(() => {
        // HTML not available, fall back to plain text if anchor is a text file
        if (BINARY_EXTENSIONS.test(anchorPath)) {
          setDocError({ key: 'preview.noHtml' })
          return
        }
        showAnchor()
      })
  ...
  useEffect(() => {
    ...
    const show = (container: HTMLElement): void => {
      ...
      const severity = parseIssue(selectedFinding.issue).severity
  ...
  if (!state || state.round === 0 || state.findings.length === 0) {
    return <div style={{ padding: '20px', color: 'rgb(102, 112, 133)' }}>{t('preview.empty')}</div>
  }
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
        ...
        {activeView === 'original' ? (
          docxError !== '' ? (
            <div style={{ color: 'rgb(217, 45, 32)', padding: '20px' }}>{t('preview.docxFailed', { message: docxError })}</div>
          ) : (
            <>
              {!docxShown && <div style={{ color: 'rgb(102, 112, 133)', padding: '20px' }}>{t('preview.loading')}</div>}
          ...
          <div style={{ color: 'rgb(217, 45, 32)', padding: '20px' }}>{t(docError.key, { message: docError.message ?? '' })}</div>
        ) : (
          <div style={{ color: 'rgb(102, 112, 133)', padding: '20px' }}>{t('preview.loading')}</div>
        )}
      </div>
    </div>
  )
